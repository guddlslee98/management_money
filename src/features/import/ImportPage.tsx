...
export default function ImportPage() {
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        setMapping(saved ?? det.profile.mapping(table.headers))
        ...
        setOverrides(new Map())
        setResult(null)
        const acc = guessAccount(accounts, det.profile.hint)
        setAccountId(acc)
  ...
  const runImport = useCallback(async () => {
    if (!loaded || !mapping || !accountId) return
    setImporting(true)
    setError(null)
    try {
      ...
      rows.forEach((r, i) => {
        ...
        const categoryId = isTransfer ? null : overrides.has(r.rowIndex) ? (overrides.get(r.rowIndex) ?? null) : suggestions[i]
      ...
      setResult({ added: inputs.length, duplicates: skippedDup, errors, latestMonth: latest })
      setStep(4)
    } catch (e) {
  ...
  }, [loaded, mapping, accountId, counterAccountId, rows, selected, overrides, suggestions, hashes, duplicates])
